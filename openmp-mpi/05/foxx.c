...
int main (int argc, char *argv[]) {
  ...
  for(k = 0; k < pn; ++k) {
    ...
    // shifting
    MPI_Irecv(Bb_temp, blocksize, MPI_DOUBLE, colrank == pn-1 ? 0 : colrank+1, 31234, proc_col, &req1);
    MPI_Isend(Bb, blocksize, MPI_DOUBLE, colrank == 0 ? pn-1 : colrank-1, 31234, proc_col, &req2);
